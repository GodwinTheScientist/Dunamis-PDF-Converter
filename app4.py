            ...
            for idx, file in enumerate(st.session_state.uploaded_files):
                ...
                for prayer in prayers:
                    ...
                    if m:
                        ...
                        # Start at maximum chosen slider font size (e.g. 64pt) and only downscale if character density dictates it.
                        current_body_size = body_size
                        char_count = len(text_content)
                        
                        if char_count > 250:
                            current_body_size = max(32, int(body_size * 0.55)) # Scale down heavily for multi-line block overlays
                        elif char_count > 130:
                            current_body_size = max(40, int(body_size * 0.75)) # Scale down moderately for medium paragraphs
                        elif char_count < 60:
                            current_body_size = min(80, int(body_size * 1.2))  # Scale short content even larger up to 80pt max!

                        # Dynamic Y adjustment to lift short sentences closer to center
                        box_top = 2.4 if char_count > 130 else 3.0
                        box_height = 4.6 if char_count > 130 else 3.5

                        add_centered(slide, 0.8, box_top, 11.7, box_height, text_content, current_body_size, body_color)

            bio = BytesIO()
            prs.save(bio)
            bio.seek(0)

            st.success("✅ Presentation Layouts Optimized!")
            st.download_button(
                label="⬇ Download PPTX",
                data=bio,
                file_name="Dunamis_Clean_Prayer_Points.pptx",
